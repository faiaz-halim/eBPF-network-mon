...
install-cilium-cli:
	wget https://github.com/cilium/cilium-cli/releases/download/v0.16.20/cilium-linux-amd64.tar.gz
	sudo tar xzvfC cilium-linux-amd64.tar.gz /usr/local/bin
	which cilium
	sudo rm -rf cilium-linux-amd64.tar.gz
	# cilium install --version 1.16.3
	# cilium status --wait

install-hubble-cli:
	wget https://github.com/cilium/hubble/releases/download/v1.16.3/hubble-linux-amd64.tar.gz
	sudo tar xzvfC hubble-linux-amd64.tar.gz /usr/local/bin
	which hubble
	sudo rm -rf hubble-linux-amd64.tar.gz
	# cilium hubble enable --ui

network-test-manifest:
	kubectl create -f https://raw.githubusercontent.com/cilium/cilium/1.16.3/examples/minikube/http-sw-app.yaml
	cilium hubble ui

delete-network-test-manifest:
...
up-cilium: install-nfs preparation cluster-create kubectl-config install-cilium-cli install-hubble-cli cluster-config
